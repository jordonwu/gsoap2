...
alternative for C to store collections of data.  Let's start with containers.
...
A container or dynamic array of a union requires wrapping the variant selector
and union member in a struct:
...
struct name and member name.  Also use a dynamic array instead of a container
(so you can also use this approach in C with structs):
...
A container or dynamic array of `void*` pointers to <i>`xsd:anyType`</i> data
...
of a container (you can use this in C with structs):
